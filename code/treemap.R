...
track1 <- function(file1, fileAliases = NULL, saveFolder = NULL, cutoff = 1.0, numToTrack = 5, baseFileId = 1, baseFile = NULL, colors = rainbow(5)) {
    ...
    if (baseFileId > 1) {
        print("Base file id is above 1. Defaulting to 1.")
        baseFileId <- 1
    }
    ...
    if (cutoff < 1) {
        ...
        for (index in 1:nrow(A)) {
            cur <- cur + (A$cloneCount[index] / sum)
            if (cur > cutoff) {
                cutoffIndex <- index
                break
            }
    ...
    # Create plot
    p <- ggplot(data, aes(area = cloneCount, fill = status)) +
    ...
    parameterFileName <- paste(saveFolder, "track1.csv", sep = "")
...
track2 <- function(file1, file2, fileAliases = NULL, saveFolder = NULL, cutoff = 1.0, numToTrack = 5, baseFileId = 1, baseFile = NULL, colors = rainbow(5)) {
    # Catch invalid argument values
    ...
    if (baseFileId > 2) {
        print("Base file id is above 2. Defaulting to 1.")
        baseFileId <- 1
    }
    ...
    if (cutoff < 1) {
        ...
        for (index in 1:nrow(B)) {
            cur <- cur + (B$cloneCount[index] / sum)
            if (cur > cutoff) {
                cutoffIndex <- index
                break
            }
    ...
    # Store parameters for later reference
    parameterFileName <- paste(saveFolder, "track2.csv", sep = "")
    parameter <- data.frame(
        diagrammName = c(diagrammName),
        fileAlias1 = c(fileAliases[1]),
...
track3 <- function(file1, file2, file3, fileAliases = NULL, saveFolder = NULL, cutoff = 1.0, numToTrack = 5, baseFileId = 1, baseFile = NULL, colors = rainbow(5)) {
    ...
    if (cutoff < 1) {
        ...
        for (index in 1:nrow(C)) {
            cur <- cur + (C$cloneCount[index] / sum)
            if (cur > cutoff) {
                cutoffIndex <- index
                break
            }
...
track4 <- function(file1, file2, file3, file4, fileAliases = NULL, saveFolder = NULL, cutoff = 1.0, numToTrack = 5, baseFileId = 1, baseFile = NULL, colors = rainbow(5)) {
    # Catch invalid argument values
    ...
    if (baseFileId > 4) {
        print("Base file id is above 4. Defaulting to 1.")
        baseFileId <- 1
    }
    ...
    if (cutoff < 1) {
        ...
        for (index in 1:nrow(D)) {
            cur <- cur + (D$cloneCount[index] / sum)
            if (cur > cutoff) {
                cutoffIndex <- index
                break
            }
    ...
    for (index_1 in 1:nrow(D)) {
        for (index_2 in 1:length(toTrack)) {
            if (D$id[index_1] == toTrack[index_2]) {
                D$status[index_1] <- index_2
            }
        }
    }
    ...
    # Store parameters for later reference
    parameterFileName <- paste(saveFolder, "track4.csv", sep = "")
    parameter <- data.frame(
        diagrammName = c(diagrammName),
        fileAlias1 = c(fileAliases[1]),
        fileAlias2 = c(fileAliases[2]),
