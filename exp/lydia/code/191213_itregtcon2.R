...
mid_names = c("iTreg_pool1", "NegTGFb_pool1", "Neg_pool1", "NegTreg_pool1",
              "iTreg_pool2", "NegTGFb_pool2", "Neg_pool2", "NegTreg_pool2",
              "iTreg_pool3", "NegTGFb_pool3", "Neg_pool3", "NegTreg_pool3"
          )
...
# full heatmap minus NegTreg
columns_i = grep("NegTreg", mid_names, invert=T)
columns_n = mid_names[columns_i]
data = mids_counts[columns_i]
make_full_heatmap(data, "horn",      columns_n, prefix="full_noNegTreg", binary=F, types="square", cexRow=0.9, cexCol=0.9, tsne=T)
make_full_heatmap(data, "intersect", columns_n, prefix="full_noNegTreg", types="square", vlim=c(0, NA), cexRow=0.9, cexCol=0.9)
make_full_heatmap(data, "intersect", columns_n, prefix="full_noNegTreg_top0.85", topPerc=0.85, types="square", vlim=c(0, NA), cexRow=0.9, cexCol=0.9)
...
for (tp in c(1, 0.85)) {
    ...
    run("(iTreg|NegTGFb|Neg)_pool1", "pool1_noNegTreg", save = T)
    run("(iTreg|NegTGFb|Neg)_pool2", "pool2_noNegTreg", save = T)
    run("(iTreg|NegTGFb|Neg)_pool3", "pool3_noNegTreg", save = T)
