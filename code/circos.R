...
circos2 <- function(file1, file2,
    ...
    # Set status
    for (index in 1:nrow(A)) {
        A$status[index] <- getStatus(A[index, ], 2)
    }

    # Set status
    for (index in 1:nrow(B)) {
        B$status[index] <- getStatus(B[index, ], 2)
    }
...
circos3 <- function(file1, file2, file3,
    ...
    # Set status
    for (index in 1:nrow(A)) {
        A$status[index] <- getStatus(A[index, ], 3)
    }

    # Set status
    for (index in 1:nrow(B)) {
        B$status[index] <- getStatus(B[index, ], 3)
    }

    # Set status
    for (index in 1:nrow(C)) {
        C$status[index] <- getStatus(C[index, ], 3)
    }
...
circos4 <- function(file1, file2, file3, file4,
    ...
    # Set status
    for (index in 1:nrow(A)) {
        A$status[index] <- getStatus(A[index, ], 4)
    }

    # Set status
    for (index in 1:nrow(B)) {
        B$status[index] <- getStatus(B[index, ], 4)
    }

    # Set status
    for (index in 1:nrow(C)) {
        C$status[index] <- getStatus(C[index, ], 4)
    }

    # Set status
    for (index in 1:nrow(D)) {
        D$status[index] <- getStatus(D[index, ], 4)
    }
...
circos5 <- function(file1, file2, file3, file4, file5,
    ...
    # Set status
    for (index in 1:nrow(A)) {
        A$status[index] <- getStatus(A[index, ], 5)
    }

    # Set status
    for (index in 1:nrow(B)) {
        B$status[index] <- getStatus(B[index, ], 5)
    }

    # Set status
    for (index in 1:nrow(C)) {
        C$status[index] <- getStatus(C[index, ], 5)
    }

    # Set status
    for (index in 1:nrow(D)) {
        D$status[index] <- getStatus(D[index, ], 5)
    }

    # Set status
    for (index in 1:nrow(E)) {
        E$status[index] <- getStatus(E[index, ], 5)
    }
...
circos6 <- function(file1,
    ...
    # Set status
    for (index in 1:nrow(A)) {
        A$status[index] <- getStatus(A[index, ], 6)
    }

    # Set status
    for (index in 1:nrow(B)) {
        B$status[index] <- getStatus(B[index, ], 6)
    }

    # Set status
    for (index in 1:nrow(C)) {
        C$status[index] <- getStatus(C[index, ], 6)
    }

    # Set status
    for (index in 1:nrow(D)) {
        D$status[index] <- getStatus(D[index, ], 6)
    }

    # Set status
    for (index in 1:nrow(E)) {
        E$status[index] <- getStatus(E[index, ], 6)
    }

    # Set status
    for (index in 1:nrow(F)) {
        F$status[index] <- getStatus(F[index, ], 6)
    }
